...
extension PlayingCard: CustomStringConvertible {
    var description: String {
        switch self {
        case .ace: return "\(self.rawValue)"
        case .two: return "\(self.rawValue)"
        case .three: return "\(self.rawValue)"
        case .four: return "\(self.rawValue)"
        case .five: return "\(self.rawValue)"
        case .six: return "\(self.rawValue)"
        case .seven: return "\(self.rawValue)"
        case .eight: return "\(self.rawValue)"
        case .nine: return "\(self.rawValue)"
        case .ten: return "\(self.rawValue)"
...
print(kingCard.description)

let fourCard: PlayingCard = .four
print(fourCard.description)
...
//: - Callout(Hint): There should be `52` cards in the deck. So what if you created a random number within those bounds and then retrieved that card from the deck? Remember that arrays are indexed from `0` and take that into account with your random number picking.
    extension Deck {
        func drawCard() -> Card {
            let randomCard = Int.random(in: 0..<cardArray.count)
            return cardArray[randomCard]
        }
}
...
class HighLow: CardGame {
    ...
    func play() {
        ...
        else if player 1 > player2 {
            print("Player 2 wins with \(player2.description)")
        else if player1 == player2 {
            print("Round ends in a tie with ")
                }
            }
        }
    }
